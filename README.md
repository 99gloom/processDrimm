...
+ Way 2 (Linux):
  ...
  In Linux environment, run DRIMM-Synteny using Mono in the drimm_Linux path.
  ...
  + Run DRIMM-Synteny 
    ...
    ```
    Enter the following parameters on the command line: mono command, Program.exe(DRIMM-Synteny), input path, output path, CycleLength, Dustlength.

  *Note: If it does not work, please compile the Program.cs file yourself. How to compile: After installing Mono, execute the command `msc Program.cs` to generate Program.exe*
